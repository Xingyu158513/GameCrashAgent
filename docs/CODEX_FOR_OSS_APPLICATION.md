...
API 额度将仅用于核心开源维护：对已脱敏诊断报告进行 Issue 分类和重复问题聚类；检查 PR 是否突破只读与隐私边界；为新采集器生成测试用例；自动起草变更日志和发布说明；对 PowerShell 调用、路径处理及敏感信息暴露进行安全审查。所有结论均由维护者复核，不自动修改用户系统。

## Additional information (500-character limit)

项目源于真实 Windows 游戏崩溃排查，采用标准库、只读采集、默认脱敏和保守结论设计。由于工具需要读取系统事件和驱动元数据，我们希望使用 Codex Security 持续审查命令边界、输入处理和隐私风险，并把安全规则固化在测试、贡献规范和 PR 模板中。
...
- [x] Project API credits
- [x] Conditional Codex Security access
- [x] Six months of ChatGPT Pro with Codex, if approved
